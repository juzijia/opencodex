...
- **Tool Ownership:** The CLI runs `stream-json` with built-in tools and user MCP settings disabled. When the request includes tools, OpenCodex advertises only that validated catalog through a capture-only `opencodex` MCP server; the server records call intent but never executes it. OpenCodex verifies the captured calls against the native tool calls and publishes the verified batch in native order. The host remains responsible for approval, sandboxing, and execution, and returns tool results through continuation. A missing, ambiguous, or mismatched call identity fails closed. Requests without tools keep the text-and-reasoning path; image input fails explicitly.
