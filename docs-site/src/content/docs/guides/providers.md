...
- **Tool Ownership:** The CLI runs `stream-json` with built-in tools and user MCP settings disabled. When the request includes tools, OpenCodex advertises only that validated catalog through a capture-only `opencodex` MCP server; the server records call intent but never executes it. The host receives the first verified tool call, executes it under its own approval and sandbox rules, and sends the result in the next turn. Extra parallel calls are not executed. A missing or ambiguous call identity fails closed. Requests without tools keep the text-and-reasoning path; image input fails explicitly.
