...
export interface CodingAgentTurnInput {
  ...
  /** Capture-only MCP catalog staged for this turn; the consumer owns tool-call decisions. */
...
export interface CodingAgentToolBridgeInput {
  ...
  /** CLI flag and MCP subprocess configuration, selected by the provider. */
  allowedToolsFlag?: "--allowedTools" | "--allowed-tools";
  serverArgs?: (catalogPath: string) => string[];
  serverEnv?: (bridgeDir: string) => Record<string, string>;
}

/** The common process owns these resources; provider code only consumes frames and may stop it. */
export interface CodingAgentRawTurnContext {
  frames: AsyncIterable<Record<string, unknown>>;
  bridgeDir?: string;
  emitOnce: (event: AdapterEvent) => void;
  isTerminal: () => boolean;
  kill: () => void;
  stopStream: () => void;
...
export interface CodingAgentRawTurnResult {
  sawTerminalResult: boolean;
  endError?: Extract<AdapterEvent, { type: "error" }>;
}
...
/** Run one scoped CLI process, stage its MCP catalog, expose raw frames, and reap all resources. */
export async function runCodingAgentRawTurn(
  input: CodingAgentTurnInput,
  consume: (context: CodingAgentRawTurnContext) => Promise<CodingAgentRawTurnResult>,
): Promise<void> {
        ...
        JSON.stringify({
          mcpServers: {
            [toolBridge.serverName]: {
              ...
              args: toolBridge.serverArgs?.(catalogPath) ?? codeBuddyMcpInvocation(toolBridge.serverModulePath, catalogPath),
              ...(toolBridge.serverEnv ? { env: toolBridge.serverEnv(toolBridgeDir) } : {}),
              defer_loading: false,
              alwaysLoad: true,
            },
          },
  ...
  let result: CodingAgentRawTurnResult = { sawTerminalResult: false };
  try {
    const stdin = child.stdin;
    if (stdin) {
      stdin.on("error", () => { /* EPIPE is reported through close/stderr. */ });
    ...
    try {
      result = await consume({
        frames: readJsonLines(stdout),
        bridgeDir: toolBridgeDir,
        emitOnce,
        isTerminal: () => terminalEmitted,
        kill,
        stopStream,
        profile,
        apiKey,
  ...
  if (!terminalEmitted) {
    ...
    if (incoming.abortSignal?.aborted) {
      ...
      emitOnce({ type: "error", message: `${profile.label} CLI failed to start: ${redactSecrets(childProcessError.message, profile.tokenEnv, apiKey)}`, status: 500, errorType: "upstream_error", code: "cli_spawn_failed", retryable: false });
    } else if (turnError) {
      emitOnce({ type: "error", message: redactSecrets(turnError, profile.tokenEnv, apiKey), status: 502, errorType: "upstream_error" });
    } else if (streamProtocolError) {
      emitOnce({ type: "error", message: redactSecrets(streamProtocolError, profile.tokenEnv, apiKey), status: 502, errorType: "upstream_error", code: "protocol_error", retryable: false });
      ...
      emitOnce({ type: "error", message: exitMsg, status: 502, errorType: "upstream_error", code: "process_exit_error", retryable: false });
    } else if (result.endError) {
      emitOnce(result.endError);
    } else if (!result.sawTerminalResult) {
      const msg = stderr
        ? `${profile.label} CLI ended without a terminal result frame: ${stderr}`
        : `${profile.label} CLI ended without a terminal result frame`;
      emitOnce({ type: "error", message: msg, status: 502, errorType: "upstream_error", code: "protocol_error", retryable: false });
    }
  }
}

/** CodeBuddy's existing stream-json and message_stop contract over the shared process core. */
export async function runCodingAgentTurn(input: CodingAgentTurnInput): Promise<void> {
  const { toolBridge, incoming } = input;
  await runCodingAgentRawTurn(input, async ({ frames, emitOnce, isTerminal, kill, profile, apiKey }) => {
    ...
    for await (const message of frames) {
      ...
      if (toolBridge && !isTerminal() && state.sawMessageStop && (state.completedToolCalls ?? 0) > 0) {
    ...
    return {
      sawTerminalResult: state.sawTerminalResult,
      ...(toolBridge && deferredResultDone && !state.sawMessageStop
        ? { endError: { type: "error" as const, message: `${profile.label} CLI delivered a terminal result before message_stop on a tool-bridge turn.`, status: 502, errorType: "upstream_error" as const, code: "protocol_error", retryable: false } }
        : {}),
    };
  });
}
