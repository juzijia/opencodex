...
describe("Qoder estimated usage", () => {
  ...
  test("9. price overlay uses Qoder's canonical Qwen3.8-Flash model id", () => {
    const overlay = findExpectedPriceOverlay("qoder", "Qwen3.8-Flash");
    expect(overlay).toBeDefined();
    expect(overlay?.status).toBe("verified-derived");
    expect(overlay?.cost4.input).toBe(0.16);
    expect(overlay?.cost4.output).toBe(0.47);

    // Verify resolveMatchedPrice
    const matched = resolveMatchedPrice("qoder", "Qwen3.8-Flash");
    expect(matched).toBeDefined();
    expect(matched?.cost4.input).toBe(0.16);
    expect(matched?.cost4.output).toBe(0.47);

    // Verify estimateRequestCost with estimated usage
    const cost = estimateRequestCost({
      provider: "qoder",
      model: "Qwen3.8-Flash",
      usage: { inputTokens: 1000, outputTokens: 200, totalTokens: 1200, estimated: true },
      usageStatus: "estimated",
    });
    expect(cost).toBeDefined();
    expect(cost!.cost.total).toBeGreaterThan(0);
    expect(cost!.estimated).toBe(true);
  });

  test("10. Scaffold guard integration and refusal preserved", async () => {
    const events: AdapterEvent[] = [];
    const adapter = createQoderAdapter(testProvider(), {
      which: () => "/bin/qoder",
      spawn: () =>
